...
const FloatingActions = () => {
  ...
  return (
    <>
      ...
      <a
        href={waLink('Hi ARS Financial Enterprises, I would like a free consultation.')}
        target="_blank"
        rel="noopener noreferrer"
        onClick={() => track('whatsapp_click', { location: 'floating_button' })}
        aria-label="Chat with ARS Financial Enterprises on WhatsApp"
        className={`hidden sm:flex fixed z-40 bottom-6 right-6 items-center gap-3 rounded-full bg-[#25D366] text-white font-semibold pl-4 pr-5 py-3 shadow-2xl shadow-green-900/30 transition-all duration-300 hover:scale-105 ${
      ...
      {!onContact && (
        <div className="sm:hidden fixed bottom-0 inset-x-0 z-40 grid grid-cols-3 bg-white/95 backdrop-blur border-t border-slate-200 shadow-[0_-8px_24px_rgba(15,23,42,0.12)]">
          <a
            href={telLink(SITE.phonePrimary)}
            onClick={() => track('call_click', { location: 'mobile_bar' })}
            className="flex flex-col items-center justify-center gap-0.5 py-2.5 text-primary text-xs font-semibold"
          >
          ...
          <a
            ...
            className="flex flex-col items-center justify-center gap-0.5 py-2.5 text-[#128C7E] text-xs font-semibold border-x border-slate-200"
          >
          ...
          <Link
            to="/contact"
            onClick={() => track('cta_click', { location: 'mobile_bar', label: 'Free Quote' })}
            className="flex flex-col items-center justify-center gap-0.5 py-2.5 bg-accent text-white text-xs font-semibold"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
